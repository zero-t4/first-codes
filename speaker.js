...
javascript: (function() {
    try {
        ...
        function stylesPlay() { return ` width: 45px; height: 45px; box-shadow: 1px 1px 1px black; background: url(${current}) no-repeat center; border-radius: 50%; background-color: rgba(242, 194, 138, 0.45); display: inline-block; line-height: 45px; cursor: pointer; text-align: center; bottom: 2%; right: 2%; z-index: 99999999; margin: 0 20px; `; }
...
debugger;
        ...
        document.body.onclick = function(e) {
            if (needToSpeach && e.target.id !== 'playSuperId' && e.target.id !== 'wrapperSuperId') {
                ...
                window.speechSynthesis.speak(new SpeechSynthesisUtterance(e.target.textContent));
            }
            return;
        };
