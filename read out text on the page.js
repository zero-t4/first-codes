javascript: (function() {
  try {
    ...
    document.body.onclick = function(e) {
      if (
        needToSpeach &&
        e.target.id !== "playSuperId" &&
        e.target.id !== "wrapperSuperId"
      ) {
        e.preventDefault();
        e.target.style.outline = "1px red;";
        speek = window.speechSynthesis.speak(
          new SpeechSynthesisUtterance(e.target.textContent)
        );
      }
      return;
    };
